# -*- coding: utf-8 -*-
"""
当初为了解决表格位置提取时，红色印章的干扰导致表格位置提取错误的问题！
@author: libo
"""
import sys
import cv2
...
def find_circles(img):
    take = time.time()
    # cv2.imwrite('./res_test/origin.jpg', img)
    h, w = img.shape[0:2]
    print('h1 = %s, w1 = %s' % (h, w))
    ...
    # cv2.imwrite('./res_test/img_resize.jpg', img_resize)
    ...
    cv2.imwrite('./res_test/img_resize2.jpg', img_resize)
    print('pix_mod_time', time.time() - time1)

    B_channel, G_channel, R_channel = cv2.split(img_resize)  # 注意cv2.split()返回通道顺序
    # cv2.imwrite('./res_test/B_channel.jpg', B_channel)
    # cv2.imwrite('./res_test/G_channel.jpg', G_channel)
    # cv2.imwrite('./res_test/R_channel.jpg', R_channel)
    ...
    cv2.imwrite('./res_test/binary.jpg', binary)
    ...
    for contour in contours:
        ...
        cv2.imwrite('./res_test/contours_rect.jpg', img_resize)
